...
# This will compile kubo v0.17.0 and the current sources of our plugin to match that.
...
# First, get and compile kubo v0.17.0.
...
RUN git clone https://github.com/ipfs/kubo.git . && git checkout v0.17.0
RUN go build -v -o /usr/local/bin/ipfs/ipfs-v0.17.0-docker ./cmd/ipfs
...
RUN go build -v -buildmode=plugin -o /usr/local/bin/ipfs/mexport-v0.17.0-docker.so main.go
